...
class Parameterization_CotWeight {

public:
	Parameterization_CotWeight(MeshKernel::SurfaceMesh& _mesh, size_t boundary_mode = 1, size_t vertex_index_offset = 0) :
		mesh(_mesh), BoundaryMode(boundary_mode), IndexOffset(vertex_index_offset) {
	};
	void Execute();
...
private:
	...
	std::vector<MeshKernel::VertexHandle> vex_boundary;
	std::unordered_map<MeshKernel::EdgeHandle, float> weights;
	std::vector<Eigen::Vector3f> positions;// 保存所有顶点的位置
	std::unordered_map<MeshKernel::VertexHandle, size_t> indices;// 保存所有顶点的索引
	Eigen::VectorXf B_U, B_V;

};

void Parameterization_CotWeight::Execute() {
	...
	for (auto v : mesh.allvertices()) {// 构建稀疏矩阵
		if (mesh.IsOnBoundary(v.first)) {
			...
			for (auto fh : mesh.NeighborEh(v.first)) {
				auto u = (mesh.edges(fh).vh1() == v.first) ? mesh.edges(fh).vh2() : mesh.edges(fh).vh1();
				float weight = weights[fh];
	...
	for (auto v : mesh.allvertices()) {
		MeshKernel::Vertex& vex = mesh.vertices(v.first);
		vex.setPosition(U[indices[v.first]], V[indices[v.first]], 0.f);
	}
	printf("parameterization success\n");
}

void Parameterization_CotWeight::initCotWeight() {
	for (auto edge : mesh.alledges()) {
		...
		for (auto face : mesh.NeighborFh(edge.first)) {
			...
			cot_weight += std::max(EPISLON, cot_theta);
		}
...
void Parameterization_CotWeight::findAllBoundary() {
	MeshKernel::VertexHandle first_vex, pre_vex, cur_vex;
	for (auto v : mesh.allvertices()) {
		if (mesh.IsOnBoundary(v.first)) {
			pre_vex = first_vex = v.first;
			MeshKernel::VertexHandle a, b;
			bool tag = true;
...
void Parameterization_CotWeight::setIndexOffset() {
	auto sz = vex_boundary.size();
	IndexOffset %= sz;
	if (IndexOffset == 0) return;
	std::vector<MeshKernel::VertexHandle> tmp(sz);
	for (int i = 0; i < sz; ++i) {
...
void Parameterization_CotWeight::setOriginBoundary() {
	
}

void Parameterization_CotWeight::setSquareBoundary() {
...
void Parameterization_CotWeight::setCircleBoundary() {
